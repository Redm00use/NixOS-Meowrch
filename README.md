...
> [!ВАЖНО]  
> ⚠️ Проект на текущий момент оптимизирован и протестирован только для видеокарт AMD (RADV / AMDVLK).  
> Поддержка NVIDIA (через проприетарный драйвер или nouveau) находится в процессе адаптации.  
> Если у вас NVIDIA — возможны проблемы с Hyprland, PipeWire или производительностью.  
> Следите за обновлениями репозитория для появления профиля NVIDIA.
...
- **Смена пользователя теперь через flake-параметры**: укажи имя/почту в `flake.nix` (скрипт `change-user.sh` удалён)
...
```bash
# Смена пользователя (новый способ)
Вместо скрипта укажи значения (например `username`, `fullName`, `userEmail`) в `flake.nix`.

## Применение после изменения
sudo nixos-rebuild switch --flake .#НОВЫЙ_ПОЛЬЗОВАТЕЛЬ
```
